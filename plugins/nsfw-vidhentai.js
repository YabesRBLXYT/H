...
if (!global.db.data.chats[m.chat].nsfw) return m.reply(` group does not support content *Nsfw.*`)
if (user < 18) throw m.reply(`❎ uneed to be atleast 18 years`) 
    const hasil = []
...
const handler = async (m, {
    ...
    let {
        key
    } = await conn.reply(m.chat, `🔧 Result List:\n\n${text}\n\nReply to this message with the video number you want to display.`, m)
...
handler.before = async (m, {
    ...
    if (isNaN(index) || index < 1 || index > list.length) {
        await conn.reply(m.chat, "⚠️ Enter a valid video number.", m)
    } else {
